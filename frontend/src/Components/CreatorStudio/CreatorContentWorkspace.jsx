  ...
  return (
    <section className="creator9-page creator9-content">
      <header className="creator9-page-header">
        <div>
          <span className="creator9-kicker">AUDIO</span>
          <h1>Your recordings.</h1>
          <p>Upload, find and manage the audio you have published.</p>
        </div>

        <button type="button" className="creator9-primary-button" onClick={onUpload}>
        ...
        <div className="creator9-filter-tabs">
          {['All', 'Public', 'Private'].map((item) => (
            <button
              type="button"
              key={item}
              className={visibility === item ? 'active' : ''}
      ...
      {loading ? (
        <div className="creator9-row-loading"><span /><span /><span /><span /></div>
      ) : filtered.length === 0 ? (
        <div className="creator9-empty">
          <EchoSignal size="lg" state="idle" activeNodes={0} />
          <h2>{tracks.length ? 'No matching audio' : 'No audio yet'}</h2>
          <p>
            {tracks.length
          ...
          {filtered.map((track, index) => {
            ...
            return (
              <article key={id || index}>
                ...
                <div className="creator9-content-stat"><FaPlay /><span>{formatNumber(track.plays)}</span></div>
                <div className="creator9-content-stat"><FaHeart /><span>{formatNumber(track.likes)}</span></div>
                <div className="creator9-content-date">{formatDate(track.createdAt)}</div>

                <span className={`creator9-visibility ${track.isPublic ? 'public' : 'private'}`}>
                  {track.isPublic ? <FaGlobe /> : <FaLock />}
                ...
                <button
                  ...
                  onClick={() => onDelete(id, track.title)}
