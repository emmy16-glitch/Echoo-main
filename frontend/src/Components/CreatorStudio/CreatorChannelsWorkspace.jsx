  ...
  return (
    ...
    >
      ...
      <div className="channels-card-body">
        ...
        <footer>
          <span><FiRadio aria-hidden="true" /> {formatCount(row.listenerCount)} listening</span>
...
const FilterGroup = ({ icon, title, children }) => (
  <section className="channels-filter-group">
    <header><span>{icon}</span><strong>{title}</strong><FiChevronDown aria-hidden="true" /></header>
    {children}
  </section>
);
...
export default function CreatorChannelsWorkspace() {
  ...
  useEffect(() => {
    ...
    const refreshPresence = async (broadcastId) => {
      try {
        ...
        // The canonical discovery count remains visible if presence sync is unavailable.
      }
    };
  ...
  const languages = useMemo(() => [...new Set(rows.map((row) => row.languageCode).filter(Boolean))]
    .filter((code) => CHANNEL_LANGUAGE_NAMES[code]), [rows]);
  ...
  return (
    <section className="channels-page">
      <header className="channels-page-heading"><h1>Channels</h1><p>Discover and listen to live stations on Echoo.</p></header>
      <section className="channels-surface">
        <div className="channels-toolbar">
          <label className="channels-search"><FiSearch aria-hidden="true" /><span className="channels-sr-only">Search channels or topics</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search channels or topics..." />{query && <button type="button" onClick={() => setQuery('')} aria-label="Clear channel search"><FiX /></button>}</label>
          <div className="channels-toolbar-actions">
            <label className="channels-sort"><span className="channels-sr-only">Sort channels</span><select value={sort} onChange={(event) => setSort(event.target.value)}><option value="live">Sort by: Live now</option><option value="listeners">Sort by: Most listeners</option><option value="newest">Sort by: Newest</option></select></label>
            <div className="channels-view-toggle" aria-label="Channel view"><button type="button" className={view === 'grid' ? 'active' : ''} aria-pressed={view === 'grid'} aria-label="Grid view" onClick={() => setView('grid')}><FiGrid /></button><button type="button" className={view === 'list' ? 'active' : ''} aria-pressed={view === 'list'} aria-label="List view" onClick={() => setView('list')}><FiList /></button></div>
        ...
        <div className="channels-layout">
          <main className="channels-results">
            <header className="channels-section-heading"><span><FiRadio /></span><div><h2>Live now</h2><p>Stations that are currently live</p></div></header>
            {error ? (
              ...
              <div className="channels-state"><FiRadio aria-hidden="true" /><strong>{rows.some((row) => row.isLive) ? 'No channels found.' : 'No channels are live right now.'}</strong><p>{rows.some((row) => row.isLive) ? 'Try adjusting your search or filters.' : 'Check back soon.'}</p>{(query || category !== 'All' || language !== 'All' || minimumAudience > 0) && <button type="button" onClick={clearFilters}>Clear filters</button>}</div>
            )}
          </main>
          <aside className={`channels-filters ${filtersOpen ? 'open' : ''}`} aria-label="Channel filters">
            <h2>Filters</h2>
            <FilterGroup icon={<FiTag />} title="Categories"><div className="channels-filter-pills"><button type="button" className={category === 'All' ? 'active' : ''} aria-pressed={category === 'All'} onClick={() => setCategory('All')}>All</button>{categories.map((item) => <button type="button" key={item} className={category === item ? 'active' : ''} aria-pressed={category === item} onClick={() => setCategory(item)}>{item}</button>)}</div></FilterGroup>
            {languages.length > 0 && <FilterGroup icon={<FiHeadphones />} title="Language"><div className="channels-filter-pills"><button type="button" className={language === 'All' ? 'active' : ''} aria-pressed={language === 'All'} onClick={() => setLanguage('All')}>All</button>{languages.map((code) => <button type="button" key={code} className={language === code ? 'active' : ''} aria-pressed={language === code} onClick={() => setLanguage(code)}>{CHANNEL_LANGUAGE_NAMES[code]}</button>)}</div></FilterGroup>}
            <FilterGroup icon={<FiUsers />} title="Audience size"><div className="channels-audience"><label><span className="channels-sr-only">Minimum audience size</span><input type="range" min="0" max={audienceMaximum} value={Math.min(minimumAudience, effectiveMaximum)} onChange={(event) => setMinimumAudience(Math.min(Number(event.target.value), effectiveMaximum))} /></label><label><span className="channels-sr-only">Maximum audience size</span><input type="range" min="0" max={audienceMaximum} value={effectiveMaximum} onChange={(event) => setMaximumAudience(Math.max(Number(event.target.value), minimumAudience))} /></label><div><span>{formatCount(minimumAudience)}</span><span>{formatCount(effectiveMaximum)}{effectiveMaximum === audienceMaximum ? '+' : ''}</span></div></div></FilterGroup>
            <FilterGroup icon={<FiRadio />} title="Status"><div className="channels-filter-pills"><button type="button" className={status === 'all' ? 'active' : ''} aria-pressed={status === 'all'} onClick={() => setStatus('all')}>All</button><button type="button" className={status === 'live' ? 'active' : ''} aria-pressed={status === 'live'} onClick={() => setStatus('live')}>Live now</button></div></FilterGroup>
