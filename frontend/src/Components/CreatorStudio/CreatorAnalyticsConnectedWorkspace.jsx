...
export default function CreatorAnalyticsConnectedWorkspace() {
  ...
  return (
    <section className="analytics-reference" aria-labelledby="analytics-reference-title">
      ...
      <section className="analytics-kpis" aria-busy={loading}>{cards.map((card, index) => <article key={card.label} className="eb-card eb-card-stack"><i>{card.icon}</i><div><small>{card.label}</small><strong className="eb-number-pop" style={{ '--eb-i': index }}>{loading ? '—' : card.value}</strong><span>{loading ? 'Loading analytics…' : card.note}</span></div></article>)}</section>
