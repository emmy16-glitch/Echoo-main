...
const CreatorAnalyticsConnectedWorkspace = () => {
  ...
  return (
    <div className="b6-analytics-wrap">
      <section className="b6-analytics-trust">
        <header>
          <div>
            ...
            <h2>Performance.</h2>
            <p>See how your audio and broadcasts are performing.</p>
          </div>
        ...
        <div className="b6-metric-grid">
          <article><FaPlay /><div><strong>{number(summary.totalPlays ?? overview.totalPlays)}</strong><span>Total plays</span></div></article>
          <article><FaUsers /><div><strong>{number(overview.totalFollowers)}</strong><span>Followers</span></div></article>
          <article><FaHeadphones /><div><strong>{number(overview.avgListeners)}</strong><span>Average listeners</span></div></article>
          <article><FaChartLine /><div><strong>{number(overview.peakListeners)}</strong><span>Peak listeners</span></div></article>
          <article><FaBroadcastTower /><div><strong>{number(contentByType?.broadcasts?.count)}</strong><span>Broadcasts</span></div></article>
          <article><FaChartLine /><div><strong>{number(summary.totalTracks ?? overview.totalTracks)}</strong><span>Published audio</span></div></article>
        </div>
      </section>

      <section className="b6-analytics-trust">
        <header><div><span className="b6-kicker">TOP AUDIO</span><h2>Your most played audio.</h2></div></header>
        {topTracks.length === 0 ? (
          <div className="b6-data-boundary">No audio performance yet.</div>
        ) : (
      ...
      <section className="b6-analytics-trust">
        <header><div><span className="b6-kicker">BROADCASTS</span><h2>Recent broadcasts.</h2></div></header>
              ...
              <div className="b6-history-manage-row" key={broadcast.id || broadcast._id}>
                <div>
                  ...
                  <span>
                    ...
                    {Number(broadcast.peakListeners) > 0 ? ` · ${number(broadcast.peakListeners)} peak` : ''}
                  </span>
                </div>
