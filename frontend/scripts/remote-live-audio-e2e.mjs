...
try {
  ...
  await creatorPage.waitForFunction(() => (
    window.__echooQaRecordingEvents?.some((event) => event.status === 'done' && event.audioId)
  ), null, { timeout: 180_000 });
  const recordingEvent = await creatorPage.evaluate(() => (
    [...(window.__echooQaRecordingEvents || [])].reverse().find((event) => event.status === 'done' && event.audioId)
  ));
  audioId = String(recordingEvent?.audioId || '');
  assert.ok(audioId, 'Recording autosave completed without an audio id.');

  const serverRecording = await fetch(`${origin}/api/audio/${encodeURIComponent(audioId)}/download`, {
    headers: { Authorization: `Bearer ${token}` },
    redirect: 'follow',
  });
  assert.equal(serverRecording.ok, true, `Server recording download failed with ${serverRecording.status}.`);
  const serverMime = String(serverRecording.headers.get('content-type') || '').toLowerCase();
  const serverBytes = (await serverRecording.arrayBuffer()).byteLength;
  assert.ok(serverMime.includes('audio/mpeg'), `Expected server MP3, received ${serverMime || 'no content type'}.`);
  assert.ok(serverBytes > 1024, `Server MP3 was unexpectedly small (${serverBytes} bytes).`);

  const localDownloadDeadline = Date.now() + 15_000;
  while (!localDownloads.length && Date.now() < localDownloadDeadline) {
    await creatorPage.waitForTimeout(250);
  }
  assert.ok(localDownloads.length > 0, 'End Broadcast did not create the automatic PC safety copy.');
  assert.ok(
    localDownloads.some((filename) => /\.(wav|mp3)$/i.test(filename)),
    `Automatic PC copy did not use WAV or MP3: ${localDownloads.join(', ')}`
  );
  evidence.recording = {
    audioId,
    serverMime,
    serverBytes,
    localDownloads,
    localCopy: recordingEvent.localCopy,
  };
  evidence.ok = true;
