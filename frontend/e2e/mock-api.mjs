...
const server = http.createServer(async (req, res) => {
  ...
  if (/^\/broadcasts\/[^/]+\/processing$/.test(path)) {
    const broadcastId = path.split('/')[2] || '';
    const known = [broadcastLive.id, broadcastScheduled.id].includes(broadcastId);
    if (!known) {
      return json(res, 404, { error: { code: 'NOT_FOUND', message: 'Broadcast not found' } });
    }
    return json(res, 200, data({
      broadcast: {
        ...
        _id: broadcastId,
        status: 'completed',
        replayAudio: replayAudio.id,
        assetStatus: { audio: 'ready', transcript: 'ready_for_review', highlights: 'ready', chapters: 'ready' },
        assetVisibility: { audio: 'public', transcript: 'public' },
      },
      jobs: [],
    }));
  }
