...
3. **Storage:** recordings stay on local disk by default; S3-compatible bucket optional for the recording archive (`AUDIO_*` vars; replays are stored as MP3, `AUDIO_MP3_BITRATE` default `192k`). See `backend/.env.example`.
