...
import { MessageCircleQuestion, Send, ShieldCheck, X } from "lucide-react";
import {
      ...
      {isOpen && (
        <>
          <button
            ...
            className="fixed inset-0 z-[60] cursor-default bg-[#02102A]/45 backdrop-blur-[1px]"
          />
          <section
            role="dialog"
            aria-modal="true"
            ...
            className="fixed bottom-5 right-5 z-[61] flex max-h-[min(640px,calc(100dvh-2.5rem))] w-[calc(100vw-2.5rem)] max-w-md flex-col overflow-hidden rounded-[1.55rem] border border-white/15 bg-[#071A3B] text-white shadow-[0_28px_80px_rgba(2,13,35,.46)] sm:bottom-6 sm:right-6"
          >
            <header className="flex items-start justify-between gap-4 border-b border-white/10 bg-[#0B2A5B] px-5 py-4">
              ...
              <button
                type="button"
                onClick={closeAssistant}
                className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-full text-[#D8E6FF] outline-none transition hover:bg-white/10 focus-visible:ring-2 focus-visible:ring-[#91B3FF] active:scale-[.97]"
            ...
            <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-5 py-4" aria-live="polite">
              {messages.map((message, index) => (
                <article key={`${message.topic}-${index}`} className="rounded-2xl border border-white/10 bg-white/[.06] px-4 py-3">
            ...
            <div className="border-t border-white/10 bg-[#06152F] px-5 py-4">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {CURATED_HELP_SUGGESTIONS[mode].map((suggestion) => (
                  <button
                    ...
                    className="min-h-11 rounded-full border border-white/15 px-3 text-left text-[.61rem] font-bold leading-4 text-[#DCE8FF] outline-none transition hover:border-[#91B3FF] hover:bg-white/10 focus-visible:ring-2 focus-visible:ring-[#91B3FF] active:scale-[.97]"
                  >
                    {suggestion}
                  </button>
                ))}
              ...
              <form className="mt-3 flex gap-2" onSubmit={submit}>
                ...
                <button
                  type="submit"
                  disabled={!input.trim()}
                  className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-full bg-[#3B78FF] text-white outline-none transition hover:bg-[#5B8EFF] focus-visible:ring-2 focus-visible:ring-[#BFD3FF] disabled:cursor-not-allowed disabled:opacity-45 active:scale-[.97]"
