...
The original approved landing screenshots are staged outside the codebase in the author's staging area (historically `/home/ubuntu/webdev-static-assets/echoo-approved/` in that environment — not present here): `echoo-listen-dashboard.png`, `echoo-sign-in.png`, and `echoo-creator-studio.png`. To replace a landing image, stage the approved source there (or its current equivalent), upload it with the managed-media tool, then replace only the relevant `/manus-storage/...` constant. Do not move screenshots into `client/public/` or `client/src/assets/`.
